...
In machine learning historic data is used to build a model to make future predictions based on past information. Classification algorithms like the ones already discussed in this section need to validatated and tested. This section details some methods and approaches to tackling this problem so the predictions from the model are useful

\subsubsection{Resubstitution Validation}
In this scena
\citep{refaeilzadeh_cross-validation_2009}
...
Once the dataset has been constructed i.e. the experiment features are built and targets class has been labelled for each observation. The next part is deciding on how the data will be used to evaluate and test the machine learning models. 


The data needs to be broken into many partitions. This will usually include a training and testing sample datasets. Testing/Validation is done so we can evaluate of the trained model generalises well on the unseen testing/validation dataset. This is because there is a risk of over-fitting the training set.
...
Cross validation can be used when tuning the hyperparameters of the model. A lot of people seem to consider the two to be synonymous. But they are not. 
...
Cross validation is simply a way of generating training and validation sets for the process of hyperparameter tuning. Hold-out sets are also valid for hyperparameter tuning, and in fact they are computationally much cheaper.
...
One way which you combine both approaches would be if you want to fine tune some parameter or do some feature selection on you model (evaluating several models), for the final evaluation/test you might holdout dataset for validation but to choose the optimal parameter input for your model or best features this could be estimated on the training set by using cross validation.

For example if you are fine tuning your model because you are looking optimise which model to use or want to maximise or minimise a performance measure cross validation may be a good choice of evaluation.

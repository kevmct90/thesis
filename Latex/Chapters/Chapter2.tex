...
A Support Vector Machines (SVMs) learning was developed first by \cite{vapnik_nature_1995}. The algorithm performs classifications via hyperplane in a higher dimensional feature space that  aims to maximise the margin or distance separating the two classes. It can be seen in Fig. \ref{fig:svmExample} that the two class are linearly separable using the hyperplane. 
...
by finding the hyperplane that maximizes the margin between the two classes. The vectors (cases) that define the hyperplane are the support vectors.
...
\cite{vapnik_nature_1995}


\cite{shin_application_2005}

\cite{van_gestel_credit_2009}

We describe how support vector training can

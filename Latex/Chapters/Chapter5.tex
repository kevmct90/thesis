...
The results of this visualisation are quite interesting and backup the hypothesis that location is important in factor in SME defaulting in Ireland. Firstly looking at Fig. \ref{fig:June2014ArrearsByLA} we can see that the local authorities with a high percentage of default cluster to the east coast of Ireland with Cavan, Meath, Louth, Dublin performing the worst. 
...
These are strong results that the differences between the benchmark model \textit{PD\_Coarse5\_SAS} and coarse classification model \textit{PD\_Coarse5\_SAS} performance measures are so large that these macro economic features did improve the model.

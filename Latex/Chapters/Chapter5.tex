...
\small
		\begin{tabular}{l  r r r r}
			...
			\textit{NPD\_RF10\_R} & 0.556 & 0.679 & 0.618 & \cellcolor{green!25}0.658  \\ 
			\textit{NPD\_RF15\_R} & \cellcolor{green!25}0.594 & 0.690 & \cellcolor{green!25}0.643 & 0.665  \\
...
Results from the predictive models are not as promising in this experiment as there is no model that outperforms all the performance measures from the benchmark model. \textit{NPD\_RF15\_R} specifically performs quite scoring the highest score for recall and balanced accuracy. It performs significantly higher in the recall rate than the other models scoring 59.4\% compared to the next highest which is 56.1\% in model \textit{NPD\_RF20\_R}. This means it is predicting significantly more of the positive (default) cases. Interesting random forest based model perform better than the benchmark based on the recall but none perform better than the benchmark based on Specificity. This is interesting because it suggests that the macro-economic features based models will predict more of the customers that will default than the benchmark, but the benchmark model will more accurately identify customers that won't default. 

Although recall is scoring well for macro-economic based models, there does not appear to be a significant difference between the AUC which gives model score over all possible thresholds. Further testing would need to be carried out to ensure that the increase in recall is caused by the introduction of macro-economic features in the model.

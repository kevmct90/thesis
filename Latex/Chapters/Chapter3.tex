...
To link macro-economic features by electoral division and local authority to an SME customer there has to be a mechanism to link to that customers address. In an ideal world this would be done through a data model 


\begin{figure}[H]
	\includegraphics[width=1\textwidth,center]{MasterReferenceAddressDataArchitecture}
	\caption{Data Model for Mapping Customers to Electoral Division and Local Authorities}
	\label{fig:MasterReferenceAddressDataArchitecture}
\end{figure}



To create macro-economic features based on internal data in \subjectname\ there needs to be a mapping from

Address data is not something that is managed currently in \subjectname\. Customer, Account, Property Assets addresses are all stored across the databases in 

There is not master reference table 

Addresses in \subjectname\ are not recorde.
